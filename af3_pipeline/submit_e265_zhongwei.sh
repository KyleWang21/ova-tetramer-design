#!/usr/bin/env bash
# Submit the <=20-mutation cross-backbone consensus continuation on Zhongwei.
set -euo pipefail

PROJECT=/root/400083/ova_p3_tetramer_design
EXPERIMENT=experiments/e265_c4_crossbackbone_consensus_tied_design
...
[[ -s "$EXPERIMENT/target_manifest.tsv" && -f "$EXPERIMENT/SOURCE_SELECTION_FROZEN" ]] || {
  echo "missing frozen E265 target manifest" >&2
  exit 2
}
if [[ -s "$EXPERIMENT/paracloud_jobs.tsv" ]] && \
   [[ "$(tail -n +2 "$EXPERIMENT/paracloud_jobs.tsv" | wc -l)" -ge 8 ]]; then
  echo "E265 already has eight recorded Slurm submissions"
  exit 0
fi
...
for index in $(seq 0 7); do
  ...
  job_id=$(ssh -S "$CONTROL" -o "User=$USER_NAME" -p "$PORT" "$HOST" \
    "cd '$REMOTE_PROJECT' && sbatch --parsable --job-name='$task_name' \
      --export=ALL,TASK_INDEX='$index',OVA_IPTM90_EXPERIMENT='$REMOTE_PROJECT/$EXPERIMENT',OVA_IPTM90_RUN_PREFIX='run_cons2',OVA_MUTATION_BUDGET='20',OVA_MIN_SURFACE_MUTATION_FRACTION='0.80',OVA_STATE1_WEIGHT_R0='12',OVA_STATE2_WEIGHT_R0='12',OVA_STATE1_WEIGHT_R1='14',OVA_STATE2_WEIGHT_R1='10',OVA_LOGITS_ITERS='32',OVA_SOFT_ITERS='16',OVA_HARD_ITERS='8',OVA_RECYCLES='1',OVA_W_BUDGET='32',OVA_W_SPARSE='2.0',OVA_W_BURIED_MUTATION='16',OVA_WORST_STATE_GRADIENT_BOOST='1.85',OVA_SEED_OFFSET='120000' \
      af3_pipeline/run_iptm90_tied_design_paracloud.sh")
  printf '%s\t%s\t%s\t%s\t%s\n' \
    "$index" "$task_name" "$job_id" crossbackbone_consensus20_continuation_tied_af2 SUBMITTED >> "$record"
...
ssh -S "$CONTROL" -o "User=$USER_NAME" -p "$PORT" "$HOST" \
  "cd '$REMOTE_PROJECT' && setsid -f bash af3_pipeline/archive_zhongwei_tied_batch.sh \
    '$EXPERIMENT' 'run_cons2_*' '$job_ids' e265_cons2_results.tgz \
    > '$EXPERIMENT/archive.log' 2>&1 < /dev/null"
echo "E265 archive watcher started for $job_ids"
