#!/usr/bin/env bash
# Filter/rank the Rosetta-polar OVA073 tied-ProteinMPNN library and freeze E226.
set -euo pipefail
shopt -s nullglob

PROJECT=/root/400083/ova_p3_tetramer_design
E225="$PROJECT/experiments/e225_c4_073_rosetta_polar_tied_mpnn"
E226="$PROJECT/experiments/e226_c4_073_rosetta_polar_ai_af3_seed1"
POOL="$E225/candidate_pool_novel"
PY=/root/400083/antibody-design/pipeline_stage6_maskcap_bindcraft/env/bin/python
cd "$PROJECT"
if [[ -f "$E226/CANDIDATES_FROZEN" ]]; then
  echo "E226 already frozen; nothing to do"
  exit 0
fi
while [[ ! -f "$E225/REMOTE_ARCHIVE_RECOVERED" ]]; do sleep 30; done
mpnn_files=("$E225"/run_073polar1_*/mpnn_shortlist.tsv)
[[ "${#mpnn_files[@]}" -eq 8 ]] || { echo "expected 8 MPNN tables" >&2; exit 2; }
mpnn_args=(); for file in "${mpnn_files[@]}"; do mpnn_args+=(--mpnn-tsv "$file"); done

"$PY" af3_pipeline/collect_iptm90_crossmodel_candidates.py \
  --experiment "$E225" --run-glob 'no_af2_runs_*' \
  ...
  --exclude-ignore-path "$E226/manifest.tsv" \
  --out "$POOL" --top-af2 0 --top-surrogate 0 --top-mpnn 512 \
...
"$PY" af3_pipeline/prepare_ova_body_af3.py \
  ...
  --out "$E226" --top 32 --seeds 1 --seed-start 1 --seed-offset 0 \
  --shards 8 --n-chains 4
cp af3_pipeline/e226_jobs.tsv.template "$E226/jobs.tsv"
touch "$E226/CANDIDATES_FROZEN"
echo "E225 filtered, ranked, and frozen as E226 Top32"
