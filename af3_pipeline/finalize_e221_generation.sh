#!/usr/bin/env bash
# Filter/rank the AI11 AF3-Protenix cross-model tied-AF2 library.
set -euo pipefail

PROJECT=/root/400083/ova_p3_tetramer_design
E221="$PROJECT/experiments/e221_c4_ai11_af3_protenix_crossmodel_tied_design"
E222="$PROJECT/experiments/e222_c4_ai11_crossmodel_ai_af3_seed1"
POOL="$E221/candidate_pool_novel"
PY=/root/400083/antibody-design/pipeline_stage6_maskcap_bindcraft/env/bin/python
cd "$PROJECT"
if [[ -f "$E222/CANDIDATES_FROZEN" ]]; then
  echo "E222 already frozen; nothing to do"
  exit 0
fi

while [[ ! -f "$E221/REMOTE_ARCHIVE_RECOVERED" ]]; do
  sleep 30
done
"$PY" af3_pipeline/collect_iptm90_crossmodel_candidates.py \
  ...
  --exclude-ignore-path "$E222/manifest.tsv" \
...
"$PY" af3_pipeline/prepare_ova_body_af3.py \
  ...
  --out "$E222" --top 32 --seeds 1 --seed-start 1 --seed-offset 0 \
  --shards 8 --n-chains 4
cp af3_pipeline/e222_jobs.tsv.template "$E222/jobs.tsv"
touch "$E222/CANDIDATES_FROZEN"
echo "E221 filtered, ranked, and frozen as E222 Top32"
