#!/usr/bin/env bash
# Build and freeze AF3-clash-directed repairs from the high-ipTM E284 parents.
set -euo pipefail

PROJECT=/root/400083/ova_p3_tetramer_design
SOURCE="$PROJECT/experiments/e284_c4_chainmapped_eightstate_af3_seed1"
OUT="$PROJECT/experiments/e297_c4_e284_clashrepair_af3_seed1"
WORK="$PROJECT/experiments/e297_c4_e284_clashrepair_generation"
...
cd "$PROJECT"
if [[ -f "$OUT/CANDIDATES_FROZEN" ]]; then echo "E297 already frozen"; exit 0; fi
...
"$BIO_PY" af3_pipeline/plot_af3_seed1_iptm_clash_tradeoff.py \
  ...
  --out "$SOURCE/figures/e284_seed1_iptm_clash_tradeoff.png" \
  --title "E284 AF3 seed1: ipTM vs 2.4 A zero-clash reproducibility"
"$BIO_PY" af3_pipeline/prepare_e289_clash_repair_library.py \
  ...
  --exclude-glob 'experiments/e*/*/manifest.tsv' \
  --exclude-glob 'experiments/e*/*/*/manifest.tsv' \
  --out "$WORK/generation_library.tsv" --max-parents 8 --max-edits 2 \
  --max-mutations 20 --min-surface-fraction 0.80
...
"$BIO_PY" af3_pipeline/score_c4_library_hamming_ai.py \
  ...
  --out "$WORK/hamming_ai_ranking.tsv" --shortlist 32 --minimum-distance 1 \
...
echo "E297 E284 AF3 clash-directed repair candidates frozen"
