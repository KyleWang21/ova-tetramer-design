#!/usr/bin/env bash
# Filter and AF3-label-rank the OVA073 recycle-1 tied-AF2 library.
set -euo pipefail

PROJECT=/root/400083/ova_p3_tetramer_design
E219="$PROJECT/experiments/e219_c4_073_recycle1_tied_design"
E220="$PROJECT/experiments/e220_c4_073_recycle1_ai_af3_seed1"
POOL="$E219/candidate_pool_novel"
PY=/root/400083/antibody-design/pipeline_stage6_maskcap_bindcraft/env/bin/python
cd "$PROJECT"
if [[ -f "$E220/CANDIDATES_FROZEN" ]]; then
  echo "E220 already frozen; nothing to do"
  exit 0
fi

while [[ ! -f "$E219/REMOTE_ARCHIVE_RECOVERED" ]]; do
  sleep 30
done
"$PY" af3_pipeline/collect_iptm90_crossmodel_candidates.py \
  --experiment "$E219" --run-glob 'run_073r1_*' \
  ...
  --exclude-ignore-path "$E220/manifest.tsv" \
...
"$PY" af3_pipeline/prepare_ova_body_af3.py \
  ...
  --out "$E220" --top 32 --seeds 1 --seed-start 1 --seed-offset 0 \
  --shards 8 --n-chains 4
cp af3_pipeline/e220_jobs.tsv.template "$E220/jobs.tsv"
touch "$E220/CANDIDATES_FROZEN"
echo "E219 filtered, ranked, and frozen as E220 Top32"
