#!/usr/bin/env bash
# Queue eight simultaneous-four-backbone tied-AF2 designs behind E277.
set -euo pipefail

PROJECT=/root/400083/ova_p3_tetramer_design
EXPERIMENT=experiments/e280_c4_eightstate_multibackbone_tied_design
...
if [[ -s "$EXPERIMENT/paracloud_jobs.tsv" ]] && \
  ...
  echo "E280 already has eight recorded submissions"; exit 0
fi
...
rsync -az -e "ssh -S $CONTROL -o User=$USER_NAME -p $PORT" \
  ...
  af3_pipeline/run_e280_eightstate_worker.sh \
  af3_pipeline/run_e280_eightstate_paracloud.sh \
...
for index in $(seq 0 7); do
  ...
  job_id=$(ssh -S "$CONTROL" -o "User=$USER_NAME" -p "$PORT" "$HOST" \
    "cd '$REMOTE_PROJECT' && sbatch --parsable --job-name='$task_name' \
      --export=ALL,TASK_INDEX='$index',OVA_E280_EXPERIMENT='$REMOTE_PROJECT/$EXPERIMENT' \
      af3_pipeline/run_e280_eightstate_paracloud.sh")
  printf '%s\t%s\t%s\t%s\t%s\n' "$index" "$task_name" "$job_id" \
    eightstate_fourbackbone_tied_af2 SUBMITTED >> "$record"
...
ssh -S "$CONTROL" -o "User=$USER_NAME" -p "$PORT" "$HOST" \
  "cd '$REMOTE_PROJECT' && setsid -f bash af3_pipeline/archive_zhongwei_tied_batch.sh \
    '$EXPERIMENT' 'run_eightstate_*' '$job_ids' e280_eightstate_results.tgz \
    > '$EXPERIMENT/archive.log' 2>&1 < /dev/null"
echo "E280 archive watcher started for $job_ids"
