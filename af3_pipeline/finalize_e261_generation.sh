#!/usr/bin/env bash
# Freeze E262 from the adaptive E261 cross-backbone tied-AF2 trajectories.
set -euo pipefail

PROJECT=/root/400083/ova_p3_tetramer_design
E261="$PROJECT/experiments/e261_c4_e243_crossbackbone_continuation_tied_design"
E262="$PROJECT/experiments/e262_c4_e243_crossbackbone_continuation_af3_seed1"
POOL="$E261/candidate_pool_novel"
PY=/root/400083/antibody-design/pipeline_stage6_maskcap_bindcraft/env/bin/python
cd "$PROJECT"

if [[ -f "$E262/CANDIDATES_FROZEN" ]]; then
  echo "E262 already frozen; nothing to do"
  exit 0
fi
while [[ ! -f "$E261/REMOTE_ARCHIVE_RECOVERED" ]]; do sleep 30; done

"$PY" af3_pipeline/collect_iptm90_crossmodel_candidates.py \
  --experiment "$E261" --run-glob 'run_cb1_*' \
  ...
  --exclude-ignore-path "$E262/manifest.tsv" \
...
"$PY" af3_pipeline/prepare_ova_body_af3.py \
  ...
  --out "$E262" --top 32 --seeds 1 --seed-start 1 --seed-offset 0 \
  --shards 8 --n-chains 4
touch "$E262/CANDIDATES_FROZEN"
echo "E261 adaptive cross-backbone trajectories frozen as E262"
