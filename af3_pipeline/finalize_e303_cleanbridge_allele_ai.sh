#!/usr/bin/env bash
# Recombine the 13-mutation clean chassis with three higher-ipTM Accepted_AF3 donors.
set -euo pipefail

PROJECT=/root/400083/ova_p3_tetramer_design
REGISTRY="$PROJECT/experiments/current_accepted_af3_registry/accepted_af3_registry.tsv"
WORK="$PROJECT/experiments/e303_c4_cleanbridge_allele_ai_generation"
OUT="$PROJECT/experiments/e303_c4_cleanbridge_allele_ai_af3_seed1"
PY=/root/400083/antibody-design/pipeline_stage6_maskcap_bindcraft/env/bin/python
cd "$PROJECT"
if [[ -f "$OUT/CANDIDATES_FROZEN" ]]; then echo "E303 already frozen"; exit 0; fi
mkdir -p "$WORK"
"$PY" af3_pipeline/make_multihit_allele_library.py \
  ...
  --exclude-glob 'experiments/e*/*/manifest.tsv' \
  --exclude-glob 'experiments/e*/*/*/manifest.tsv' \
  --out "$WORK" --max-library 1024 --name-prefix E303-BRIDGE
...
echo "E303 clean-chassis/high-ipTM Accepted allele bridge frozen"
