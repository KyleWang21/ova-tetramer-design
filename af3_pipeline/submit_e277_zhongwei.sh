#!/usr/bin/env bash
# Submit E268 hard-hit cross-backbone continuation on eight Zhongwei A800s.
set -euo pipefail

PROJECT=/root/400083/ova_p3_tetramer_design
EXPERIMENT=experiments/e277_c4_e268_hardhit_crossbackbone_tied_design
...
if [[ -s "$EXPERIMENT/paracloud_jobs.tsv" ]] && \
  ...
  echo "E277 already has eight recorded Slurm submissions"; exit 0
fi
...
for index in $(seq 0 7); do
  ...
  job_id=$(ssh -S "$CONTROL" -o "User=$USER_NAME" -p "$PORT" "$HOST" \
    "cd '$REMOTE_PROJECT' && sbatch --parsable --job-name='$task_name' \
      --export=ALL,TASK_INDEX='$index',OVA_IPTM90_EXPERIMENT='$REMOTE_PROJECT/$EXPERIMENT',OVA_IPTM90_RUN_PREFIX='run_e268x',OVA_MUTATION_BUDGET='20',OVA_MIN_SURFACE_MUTATION_FRACTION='0.80',OVA_STATE1_WEIGHT_R0='13',OVA_STATE2_WEIGHT_R0='13',OVA_STATE1_WEIGHT_R1='15',OVA_STATE2_WEIGHT_R1='11',OVA_LOGITS_ITERS='40',OVA_SOFT_ITERS='20',OVA_HARD_ITERS='10',OVA_RECYCLES='1',OVA_W_BUDGET='34',OVA_W_SPARSE='2.2',OVA_W_BURIED_MUTATION='16',OVA_WORST_STATE_GRADIENT_BOOST='2.2',OVA_SEED_OFFSET='150000' \
      af3_pipeline/run_iptm90_tied_design_paracloud.sh")
  printf '%s\t%s\t%s\t%s\t%s\n' "$index" "$task_name" "$job_id" \
    e268_best_hard_crossbackbone_continuation_tied_af2 SUBMITTED >> "$record"
...
ssh -S "$CONTROL" -o "User=$USER_NAME" -p "$PORT" "$HOST" \
  "cd '$REMOTE_PROJECT' && setsid -f bash af3_pipeline/archive_zhongwei_tied_batch.sh \
    '$EXPERIMENT' 'run_e268x_*' '$job_ids' e277_e268x_results.tgz \
    > '$EXPERIMENT/archive.log' 2>&1 < /dev/null"
echo "E277 archive watcher started for $job_ids"
