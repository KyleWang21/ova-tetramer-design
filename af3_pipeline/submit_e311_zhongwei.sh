#!/usr/bin/env bash
# Submit eight E311 deterministic-best continuation tasks on Zhongwei A800s.
set -euo pipefail

PROJECT=/root/400083/ova_p3_tetramer_design
EXPERIMENT=experiments/e311_c4_e294_detbest_continuation_tied_design
...
if [[ -s "$EXPERIMENT/paracloud_jobs.tsv" ]] && \
  ...
  echo "E311 already has eight recorded submissions"; exit 0
fi
...
rsync -az -e "ssh -S $CONTROL -o User=$USER_NAME -p $PORT" \
  ...
  af3_pipeline/run_e286_bottomk_worker.sh \
  af3_pipeline/run_e286_bottomk_paracloud.sh \
...
for index in $(seq 0 7); do
  ...
  job_id=$(ssh -S "$CONTROL" -o "User=$USER_NAME" -p "$PORT" "$HOST" \
    "cd '$REMOTE_PROJECT' && sbatch --parsable --job-name='$task_name' \
      --export=ALL,TASK_INDEX='$index',OVA_E286_EXPERIMENT='$REMOTE_PROJECT/$EXPERIMENT' \
      af3_pipeline/run_e286_bottomk_paracloud.sh")
  printf '%s\t%s\t%s\t%s\t%s\n' "$index" "$task_name" "$job_id" \
    detbest_continuation_bottomk_tied_af2 SUBMITTED >> "$record"
...
ssh -S "$CONTROL" -o "User=$USER_NAME" -p "$PORT" "$HOST" \
  "cd '$REMOTE_PROJECT' && setsid -f bash af3_pipeline/archive_zhongwei_tied_batch.sh \
    '$EXPERIMENT' 'run_bottomk_*' '$job_ids' e311_detbest_results.tgz \
    > '$EXPERIMENT/archive.log' 2>&1 < /dev/null"
echo "E311 archive watcher started for $job_ids"
