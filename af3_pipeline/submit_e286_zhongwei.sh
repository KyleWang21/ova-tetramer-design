#!/usr/bin/env bash
# Submit eight bottom-k robust continuation tasks after E283.
set -euo pipefail

PROJECT=/root/400083/ova_p3_tetramer_design
EXPERIMENT=experiments/e286_c4_chainmapped_bottomk_tied_design
...
if [[ -s "$EXPERIMENT/paracloud_jobs.tsv" ]] && \
  ...
  echo "E286 already has eight recorded submissions"; exit 0
fi
...
for index in $(seq 0 7); do
  ...
  printf '%s\t%s\t%s\t%s\t%s\n' "$index" "$task_name" "$job_id" \
    chainmapped_bottomk_tied_af2 SUBMITTED >> "$record"
...
ssh -S "$CONTROL" -o "User=$USER_NAME" -p "$PORT" "$HOST" \
  "cd '$REMOTE_PROJECT' && setsid -f bash af3_pipeline/archive_zhongwei_tied_batch.sh \
    '$EXPERIMENT' 'run_bottomk_*' '$job_ids' e286_bottomk_results.tgz \
    > '$EXPERIMENT/archive.log' 2>&1 < /dev/null"
echo "E286 archive watcher started for $job_ids"
