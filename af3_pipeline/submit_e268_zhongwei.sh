#!/usr/bin/env bash
# Submit E251 near-discrete soft-basin stabilization on eight Zhongwei A800s.
set -euo pipefail

PROJECT=/root/400083/ova_p3_tetramer_design
EXPERIMENT=experiments/e268_c4_e251_softbasin_stabilization_tied_design
...
JOB_BASE=1224
cd "$PROJECT"
[[ -s "$EXPERIMENT/target_manifest.tsv" && -f "$EXPERIMENT/SOURCE_SELECTION_FROZEN" ]] || exit 2
if [[ -s "$EXPERIMENT/paracloud_jobs.tsv" ]] && \
   [[ "$(tail -n +2 "$EXPERIMENT/paracloud_jobs.tsv" | wc -l)" -ge 8 ]]; then
  echo "E268 already has eight recorded Slurm submissions"; exit 0
fi
...
for index in $(seq 0 7); do
  ...
  job_id=$(ssh -S "$CONTROL" -o "User=$USER_NAME" -p "$PORT" "$HOST" \
    "cd '$REMOTE_PROJECT' && sbatch --parsable --job-name='$task_name' \
      --export=ALL,TASK_INDEX='$index',OVA_IPTM90_EXPERIMENT='$REMOTE_PROJECT/$EXPERIMENT',OVA_IPTM90_RUN_PREFIX='run_softstab',OVA_MUTATION_BUDGET='20',OVA_MIN_SURFACE_MUTATION_FRACTION='0.80',OVA_STATE1_WEIGHT_R0='12',OVA_STATE2_WEIGHT_R0='12',OVA_STATE1_WEIGHT_R1='14',OVA_STATE2_WEIGHT_R1='10',OVA_LOGITS_ITERS='48',OVA_SOFT_ITERS='24',OVA_HARD_ITERS='12',OVA_RECYCLES='1',OVA_W_BUDGET='32',OVA_W_SPARSE='2.0',OVA_W_BURIED_MUTATION='16',OVA_WORST_STATE_GRADIENT_BOOST='2.0',OVA_SEED_OFFSET='130000' \
      af3_pipeline/run_iptm90_tied_design_paracloud.sh")
  printf '%s\t%s\t%s\t%s\t%s\n' "$index" "$task_name" "$job_id" \
    e251_neardiscrete_softbasin_stabilization SUBMITTED >> "$record"
...
ssh -S "$CONTROL" -o "User=$USER_NAME" -p "$PORT" "$HOST" \
  "cd '$REMOTE_PROJECT' && setsid -f bash af3_pipeline/archive_zhongwei_tied_batch.sh \
    '$EXPERIMENT' 'run_softstab_*' '$job_ids' e268_softstab_results.tgz \
    > '$EXPERIMENT/archive.log' 2>&1 < /dev/null"
echo "E268 archive watcher started for $job_ids"
