#!/usr/bin/env bash
# Optimize one tied sequence against four chain-remapped AF3 backbones / eight interfaces.
set -euo pipefail
...
EXPERIMENT="${OVA_E283_EXPERIMENT:-$PROJECT_ROOT/experiments/e283_c4_chainmapped_eightstate_tied_design}"
cd "$PROJECT_ROOT"
...
run="$EXPERIMENT/run_chainmapped_$(printf '%02d' "$TASK_INDEX")"
joint="$run/joint_CANONICAL12_r$((TASK_INDEX % 2))"
...
echo "task=$TASK_INDEX anchor=$anchor shared_tied_sequence=1 contexts=4 canonical_interface_states=8"
"$BINDCRAFT_ROOT/env/bin/python" "$PROJECT_ROOT/af3_pipeline/af2_multistate_tied_lowmut_design.py" \
  ...
  --seed "$((181201 + TASK_INDEX))" --mutation-budget 20 \
