#!/usr/bin/env bash
# Tied ProteinMPNN sampling on four clean AF3 backbones from candidate 276.
set -euo pipefail
...
SOURCE="${OVA_MPNN_SOURCE:-$PROJECT_ROOT/experiments/e204_c4_276_strictensemble_tied_design}"
EXPERIMENT="${OVA_MPNN_EXPERIMENT:-$PROJECT_ROOT/experiments/e205_c4_276_strictensemble_tied_mpnn}"
...
"$BINDCRAFT_ROOT/env/bin/python" "$PROJECT_ROOT/af3_pipeline/mpnn_tied_c4_lowmut_design.py" \
  ...
  --seed "$((20260901 + TASK_INDEX + ${OVA_MPNN_SEED_OFFSET:-38000}))" \
  --min-mutations 12 --max-mutations 20 \
  --min-surface-fraction 0.80 --shortlist 128

echo "candidate-276 ProteinMPNN task $TASK_INDEX complete: $out"
