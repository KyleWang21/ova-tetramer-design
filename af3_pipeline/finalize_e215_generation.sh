#!/usr/bin/env bash
# Filter and AF3-label-rank the OVA-C4-073 balanced-v9 tied-AF2 library.
set -euo pipefail

PROJECT=/root/400083/ova_p3_tetramer_design
E215="$PROJECT/experiments/e215_c4_073_balanced_tied_design"
E216="$PROJECT/experiments/e216_c4_073_balanced_v9_ai_af3_seed1"
POOL="$E215/candidate_pool_novel"
PY=/root/400083/antibody-design/pipeline_stage6_maskcap_bindcraft/env/bin/python
cd "$PROJECT"
if [[ -f "$E216/CANDIDATES_FROZEN" ]]; then
  echo "E216 already frozen; nothing to do"
  exit 0
fi

while [[ ! -f "$E215/REMOTE_ARCHIVE_RECOVERED" ]]; do
  sleep 30
done
"$PY" af3_pipeline/collect_iptm90_crossmodel_candidates.py \
  --experiment "$E215" --run-glob 'run_073v9_*' \
  ...
  --exclude-ignore-path "$E216/manifest.tsv" \
...
"$PY" af3_pipeline/prepare_ova_body_af3.py \
  ...
  --out "$E216" --top 32 --seeds 1 --seed-start 1 --seed-offset 0 \
  --shards 8 --n-chains 4
cp af3_pipeline/e216_jobs.tsv.template "$E216/jobs.tsv"
touch "$E216/CANDIDATES_FROZEN"
echo "E215 filtered, ranked, and frozen as E216 Top32"
