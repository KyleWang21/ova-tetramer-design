#!/usr/bin/env bash
# Submit the active-learning-seeded tied-AF2 wave to Zhongwei (8x one GPU).
set -euo pipefail

PROJECT=/root/400083/ova_p3_tetramer_design
EXPERIMENT=experiments/e235_c4_crossmodel_active_tied_design
...
[[ -s "$EXPERIMENT/target_manifest.tsv" ]] || {
  echo "missing frozen E235 target manifest" >&2
  exit 2
}
if [[ -s "$EXPERIMENT/paracloud_jobs.tsv" ]] && \
   [[ "$(tail -n +2 "$EXPERIMENT/paracloud_jobs.tsv" | wc -l)" -ge 8 ]]; then
  echo "E235 already has eight recorded Slurm submissions"
  exit 0
fi
...
for index in $(seq 0 7); do
  ...
  job_id=$(ssh -S "$CONTROL" -o "User=$USER_NAME" -p "$PORT" "$HOST" \
    "cd '$REMOTE_PROJECT' && sbatch --parsable --job-name='$task_name' \
      --export=ALL,TASK_INDEX='$index',OVA_IPTM90_EXPERIMENT='$REMOTE_PROJECT/$EXPERIMENT',OVA_IPTM90_RUN_PREFIX='run_xm2',OVA_MUTATION_BUDGET='22',OVA_MIN_SURFACE_MUTATION_FRACTION='0.80',OVA_STATE1_WEIGHT_R0='10',OVA_STATE2_WEIGHT_R0='12',OVA_STATE1_WEIGHT_R1='12',OVA_STATE2_WEIGHT_R1='10',OVA_LOGITS_ITERS='28',OVA_SOFT_ITERS='14',OVA_HARD_ITERS='8',OVA_RECYCLES='1',OVA_W_BUDGET='24',OVA_W_SPARSE='1.8',OVA_W_BURIED_MUTATION='14',OVA_SEED_OFFSET='80000' \
      af3_pipeline/run_iptm90_tied_design_paracloud.sh")
  printf '%s\t%s\t%s\t%s\t%s\n' \
    "$index" "$task_name" "$job_id" active_learning_seeded_tied_af2 SUBMITTED >> "$record"
